...
library DynamicArrayLib {
    ...
    function slice(uint256[] memory a, uint256 start) internal pure returns (uint256[] memory result) {
    ...
    function resize(DynamicArray memory a, uint256 n) internal pure returns (DynamicArray memory result) {
        ...
        assembly {
            ...
            if iszero(lt(n, arrLen)) {
                calldatacopy(add(arrData, shl(5, add(1, arrLen))), calldatasize(), shl(5, sub(n, arrLen)))
            }
            mstore(arrData, n)
        }
    }
