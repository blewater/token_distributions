...
abstract contract TimedRoles {
    ...
    event TimedRoleSet(address indexed holder, uint256 indexed timedRole, uint40 start, uint40 expires);
    ...
    function setTimedRole(address holder, uint256 timedRole, uint40 start, uint40 expires) public payable virtual {
    ...
    function _setTimedRole(address holder, uint256 timedRole, uint40 start, uint40 expires) internal virtual {
    ...
    function _authorizeSetTimedRole(address holder, uint256 timedRole, uint40 start, uint40 expires) internal virtual {
        ...
        returns (bool result)
    {
        ...
        assembly {
            ...
            for {} lt(result, lt(encodedTimeRoles, end)) {} {
                ...
                let p := sload(keccak256(0x00, 0x38))
                result := iszero(or(lt(timestamp(), shr(216, p)), gt(timestamp(), and(0xffffffffff, p))))
            }
        }
    }
