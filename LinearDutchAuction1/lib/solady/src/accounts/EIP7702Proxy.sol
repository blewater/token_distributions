        ...
        assembly {
            ...
            if iszero(xor(address(), s)) {
                ...
                if eq(0x5c60da1b, fnSel) {
                    if staticcall(gas(), address(), calldatasize(), 0x00, 0x00, 0x20) { return(0x00, returndatasize()) }
                }
            ...
            if iszero(
                and( // The arguments of `and` are evaluated from right to left.
                    delegatecall(gas(), mload(calldatasize()), 0x00, calldatasize(), calldatasize(), 0x00),
