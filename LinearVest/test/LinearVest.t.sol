...
contract LinearVestTest is Test {
    ...
    function test_createVest_event() public {
        ...
        vm.prank(alice);
        vest.createVest(rareToken, bob, 100e18, uint40(block.timestamp + 1 days), uint40(100 days), 0);
    }
    ...
    function test_createVest_feeOnTransfer() public {
        ...
        vm.expectRevert();
        vm.prank(alice);
        vest.createVest(rareTokenFeeOnTransfer, bob, 100e18, uint40(block.timestamp + 1 days), uint40(100 days), 0);
    }

    function test_createVestParameterSanity() public {
        vm.expectRevert();
        vm.prank(alice);
        vest.createVest(IERC20(address(0)), bob, 100e18, uint40(block.timestamp + 1 days), uint40(100 days), 0);

        vm.expectRevert();
        vm.prank(alice);
        vest.createVest(rareToken, address(0), 100e18, uint40(block.timestamp + 1 days), uint40(100 days), 0);

        vm.expectRevert();
        vm.prank(alice);
        vest.createVest(rareToken, bob, 0, uint40(block.timestamp + 1 days), uint40(100 days), 0);

        vm.expectRevert();
        vm.prank(alice);
        vest.createVest(rareToken, bob, 100e18, uint40(block.timestamp - 1 seconds), uint40(100 days), 0);

        vm.expectRevert();
        vm.prank(alice);
        vest.createVest(rareToken, bob, 100e18, uint40(block.timestamp + 1 days), uint40(0), 0);
    }
    ...
    function test_fullVest() public {
        ...
        bytes32 vestId =
            vest.computeVestId(rareToken, bob, 100e18, uint40(block.timestamp + 1 days), uint40(100 days), 0);
    ...
    function test_100SecondVest() public {
        vm.prank(alice);
        vest.createVest(rareToken, bob, 100e18, uint40(block.timestamp), uint40(100 seconds), 0);
    ...
    function test_100MinuteVest() public {
        vm.prank(alice);
        vest.createVest(rareToken, bob, 100e18, uint40(block.timestamp), uint40(100 minutes), 0);
